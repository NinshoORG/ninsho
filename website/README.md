...
The product site — what Ninsho is, what it stops, what it costs, how it works
step by step, and how to run the demonstration.
...
| GitHub Pages | `.github/workflows/pages.yml` deploys this folder on every push to `main` that touches it. One-time setup: Settings → Pages → Source: **GitHub Actions**. (Branch-based Pages can only serve `/` or `/docs`, so a folder setting cannot do this.) |
...
So: a handful of pages, one stylesheet, and two small scripts with no
dependencies. `app.js` remembers a theme choice, marks the current page in the
nav and wires the copy buttons; the site degrades to a readable one if it never
runs. `walkthroughs-player.js` renders the recorded walkthroughs and is loaded
on that one page only.

## The walkthroughs

`walkthroughs.html` replays recordings of the real library: an app built on
`@ninshorg/server`, sent real HTTP requests, with every status code, store
operation and audit event captured. The recordings live in
`assets/walkthroughs.js` and are **generated — never edit that file**:

```bash
npm run build          # the recorder uses the built packages
npm run walkthroughs   # re-record → website/assets/walkthroughs.js
```

Three things keep them honest, all in
`examples/playground/src/walkthroughs.ts`: each step declares the status and
audit events it expects before it runs, and the recorder refuses to write a
trace that disagrees; no raw token may appear in any store operation; and every
test a step cites must exist. CI runs `npm run walkthroughs:check` on every
push, which records them again and fails if the library no longer behaves the
way the page shows.

The file is loaded with a plain `<script>` rather than `fetch()`, so the page
...
walkthroughs.html       seven attacks, recorded from the real library
performance.html        measured numbers, with the hardware stated
demo.html               the ten-panel playground tour
brand.html              the identity, and how to use it
404.html
assets/
  style.css             every token and component
  app.js                theme toggle, current-page marking, copy buttons
  walkthroughs.js       GENERATED by `npm run walkthroughs` — do not edit
  walkthroughs-player.js  renders the recordings; walkthroughs.html only
  brand/                the mark, lockups, icons and social card
robots.txt
sitemap.xml
```
