...
- **A wider performance evaluation.** Contributed by Yash Jadhav.
  ...
  verification, concurrency scaling from 1 to 100 workers, and end-to-end HTTP
  through the Express 5 example.

  It audits the existing suites rather than replacing them, which was the right
  call — `packages/server/bench` and `packages/webauthn/bench` are what CI
  runs, and they stay the regression gate.

  Measured on an Intel i5-4300U, which turns out to matter. See *Changed*.
...
- **The two performance documents now explain each other.** `PERFORMANCE.md`
  reports `opaque: verify` as ~15× faster than `paseto: verify`;
  `BENCHMARK-REPORT.md` reports ~5.9×. A reader finding both would reasonably
  conclude one was wrong.

  Neither is. They are different machines — an i7-8665U on Node 24 against an
  i5-4300U on Node 22 — and `PERFORMANCE.md` did not say which hardware it used,
  so there was nothing to reconcile against. It does now, and an independent
  re-run on the same class of machine reproduced it: 105,748 against 6,840, a
  ratio of 15.5×.

  The asymmetry is the part worth keeping. Both figures fall on the slower CPU,
  but not equally — the opaque path loses about 6× and Ed25519 verification only
  about 2.7×, because Ed25519 is a tight OpenSSL routine that tracks clock speed
  while the opaque path is comparatively more V8 work that suffers on an older
  dual-core with a smaller cache.

  So the honest claim is **between roughly 6× and 15×, depending on the
  machine**, and both documents now say so. The conclusion is unchanged — opaque
  is faster on every machine measured — but a single multiple stated as a
  property of the library was not something either document could support.
...
- **A benchmark was timing a verification with two of its checks disabled.**
  `dpop-bench.ts` called `verifyDpopProof` without `maxAgeSeconds` or
  `clockToleranceSeconds`, both of which `VerifyProofOptions` marks required.

  At runtime `undefined * 1000` is `NaN`, and every comparison against `NaN` is
  false — so the "issued in the future" and "proof is too old" checks never
  fired. Confirmed rather than reasoned about: a proof issued eleven days ago
  passes the age check under those options and is correctly rejected under the
  library's defaults.

  The published figures stand, and that was measured too rather than assumed.
  Running both versions back to back on one machine moved
  `verifyDpopProof (ES256)` by +0.1% and `(EdDSA)` by −1.5%, within run-to-run
  noise, because two integer comparisons cost nothing beside a signature
  verification.

  What did not stand is that nothing caught it, which is the reason it is
  recorded at all.

- **`benchmarks/` was outside every type check**, exactly as `manualtest/` had
  been — not a workspace, no `tsconfig.json`, compiled by nothing. It now has
  one, chained into the root `typecheck` so CI compiles it on every push, and
  the gate was verified by breaking a file on purpose and watching it fail.

  Three errors surfaced, including the DPoP one above. That is twice now that
  adding a type check to an unchecked top-level directory has found a real
  defect, which is recorded in `AGENTS.md` as a trap rather than left to be
  rediscovered a third time.

- **The published benchmark results could not be regenerated by the script that
  produced them.** `dpop-bench.ts` labelled its rows `Ed25519` while the
  committed JSON and the report both said `EdDSA`, so the two had drifted apart.

  `EdDSA` is the correct name — it is what `DpopAlgorithm` declares, and what
  the script passes to the API — so the labels moved to match rather than the
  evidence.

- **Two benchmark rows were quoted to six significant figures they cannot
  support.** `jwkThumbprint` and `accessTokenHash` take about 3 µs, close
  enough to timer and scheduler noise that repeat runs disagree sharply: two
  consecutive runs on one machine, with nothing changed that could affect
  either, swung −38% and +76%. They are now given to two significant figures
  with the variance stated. The claim they support — negligible beside a
  signature verification — is unaffected.

- **The benchmark report leaked absolute paths from the author's machine.**
  Three links pointed at a `file:///c:/.../ninsho/...` path, which is dead for
  every other reader and disclose a local directory structure. Rewritten as
  repo-relative links, which was the last thing in that document that would not
  have survived the repository being public.

- **`PERFORMANCE.md` quoted figures that appear in none of its tables.** The
  prose supporting the ~15× claim cited 122,934 against 8,116; the tables beside
  it say 110,355 and 8,001. Left behind by a superseded run. The conclusion was
  unaffected, which is exactly why it survived — a number in the prose that its
  own evidence does not support is the failure this project exists around, and
  it is easiest to miss when it happens to be pointing the right way.
