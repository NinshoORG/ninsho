 ...
 *
 * ─── Why this is in src/ and not shipped ──────────────────────────────────
 * Nothing here is exported from `index.ts`, and `package.json` publishes only
 * `dist`, so none of it reaches a consumer. It lives in `src/` so it
 * type-checks under the same strict settings as the code it tests — a test
 * helper that is allowed to be sloppy will eventually hide a real defect.
 *
 * The encoder exists because the decoder must be tested against inputs the
 * decoder itself did not produce, including inputs no real authenticator would
 * ever emit. The virtual authenticator exists because the ceremony code is
 * only meaningfully tested against signatures that were actually generated by
 * a private key — asserting that a verifier accepts a response the verifier
 * itself constructed proves nothing at all.
 * ──────────────────────────────────────────────────────────────────────────
 */
